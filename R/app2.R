    ...
    tabItems(
      ...
      tabItem(tabName = "vis",
              ...
              fluidRow(
                column(width = 6, 
                       box(width = NULL, solidHeader = TRUE, 
                           plotlyOutput("grafico_series_est", height = 500)
                       )
                ),
              ...
              fluidRow(
                column(width = 6, 
                       box(width = NULL, solidHeader = TRUE, 
                           plotlyOutput("grafico_series_est1", height = 500)
                       )
                ),
              ...
              fluidRow(
                column(width = 6, 
                       box(width = NULL, solidHeader = TRUE, 
                           plotlyOutput("grafico_series_est2", height = 500)
                       )
                ),
...
server <- function(input, output, session) {
  ...
  output$grafico_series_est <- renderPlotly({
    est <- input$e_c
    cid <- input$cidade_filtro
    if(is.null(est) || est == '') {
      df_cidade <- covid19(country = c('Brazil'), level=1, verbose=F)  %>% filter(date >= input$date_slider[1],
    ...
    p <- grafico_series_estacionaria(df_cidade$date, df_cidade$confirmed, titulo, "Data", "Novos Confirmados Diários")
  ...
  output$grafico_ACF <- renderPlot({
    ...
    tendencias <- estima_tendencia(df_cidade$confirmed)
    
    df_cidades <- df_cidade %>%
      slice(-1) %>%
      mutate(confirmed = diff(df_cidade$confirmed)) %>%
      mutate(sem_tendencia = confirmed - diff(tendencias))
    
    #Autocorrelação depois de retirar a tendência
    
    dados = tsibble(
      data = df_cidades$date,
      y = df_cidades$sem_tendencia,
      index = data
    )
  ...
  output$grafico_series_est1 <- renderPlotly({
    est <- input$e_c
    cid <- input$cidade_filtro
    if(is.null(est) || est == '') {
      df_cidade <- covid19(country = c('Brazil'), level=1, verbose=F)  %>% filter(date >= input$date_slider[1],
    ...
    p <- grafico_series_estacionaria(df_cidade$date, df_cidade$deaths, titulo, "Data", "Novos Confirmados Diários")
  ...
  output$grafico_ACF1 <- renderPlot({
    ...
    tendencias <- estima_tendencia(df_cidade$deaths)
    
    df_cidades <- df_cidade %>%
      slice(-1) %>%
      mutate(deaths = diff(df_cidade$deaths)) %>%
      mutate(sem_tendencia = deaths - diff(tendencias))
  ...
  output$grafico_series_est2 <- renderPlotly({
    est <- input$e_c
    cid <- input$cidade_filtro
    if(is.null(est) || est == '') {
      df_cidade <- covid19(country = c('Brazil'), level=1, verbose=F)  %>% filter(date >= input$date_slider[1],
    ...
    # print(df_cidade$vaccines)
    p <- grafico_series_estacionaria(df_cidade$date, df_cidade$vaccines/10000, titulo, "Data", "Novos Confirmados Diários")
  ...
  output$grafico_ACF2 <- renderPlot({
    ...
    tendencias <- estima_tendencia(df_cidade$vaccines)
    
    df_cidades <- df_cidade %>%
      slice(-1) %>%
      mutate(vaccines = diff(df_cidade$vaccines)) %>%
      mutate(sem_tendencia = vaccines - diff(tendencias))
